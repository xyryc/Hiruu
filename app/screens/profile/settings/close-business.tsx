...
const CloseBusiness = () => {
  ...
  const handleConfirmDeleteCompany = async () => {
    ...
    if (!targetBusinessId) {
      toast.error(t("user.profile.noBusinessToDelete"));
      return;
    }
    ...
    try {
      ...
      useBusinessStore.setState((state) => ({
        myEmployments: (Array.isArray(state.myEmployments) ? state.myEmployments : []).filter(
          (employment: any) =>
            (employment?.business?.id || employment?.businessId) !== targetBusinessId
        ),
      }));
      setSelectedBusinesses([]);
      void getMyEmployments(true).catch(() => undefined);
      ...
      >
        ...
        <View className="absolute bottom-10 left-5 right-5">
          <PrimaryButton
            ...
            loading={isDeleting}
            className={`${!canProceed || isDeleting ? "opacity-50" : ""}`}
          />
        </View>
